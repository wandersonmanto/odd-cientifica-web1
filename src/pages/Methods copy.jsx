...
import { Target, Save, BarChart2, Check, X, AlertCircle, ChevronDown, ChevronUp, Activity } from 'lucide-react';
...
const Methods = () => {
  ...
  // Estado para expandir detalhes do jogo
  ...
  useEffect(() => {
    fetchData();
    setExpandedRow(null); // Fecha detalhes ao trocar data/método
  }, [activeMethod, selectedDate]);

  const fetchData = async () => {
    setLoading(true);
    try {
      // Query expandida com TODAS as estatísticas solicitadas
  ...
  const toggleMainPick = async (entry) => {
    if (dailyResult && dailyResult.status !== 'pending') {
  ...
  const handleSaveStake = async () => {
    try {
        await saveDailyStake(activeMethod, selectedDate, dailyStake, activeMethod === 'my_games' ? dailyOdd : null);
        alert("Configuração do dia salva!");
    } catch (e) {
  ...
  const handleCloseDay = async () => {
      try {
          await closeDailyResult(activeMethod, selectedDate, entries, dailyStake, dailyOdd);
          fetchData(); 
      } catch (e) {
          alert(e.message);
      }
  ...
  if (showExtract) {
      return <MethodExtract method={activeMethod} onBack={() => setShowExtract(false)} />;
  }
    ...
    <div className="p-6 h-full flex flex-col">
          ...
          <div className="flex items-end gap-4 flex-wrap">
              <div className="flex flex-col">
                  ...
                  <input 
                    ...
                    className="bg-brand-dark border border-brand-gray text-white rounded p-2 outline-none h-10"
                  />
              </div>
              ...
              <div className="flex flex-col">
                  ...
                  <input 
                    ...
                    className="bg-brand-dark border border-brand-gray text-white font-mono rounded p-2 w-28 outline-none h-10"
                    placeholder="0.00"
                    disabled={dailyResult && dailyResult.status !== 'pending'}
                  />
              </div>
              ...
              {activeMethod === 'my_games' && (
                  <div className="flex flex-col">
                      ...
                      <input 
                        type="number" 
                        value={dailyOdd} 
                        onChange={e => setDailyOdd(e.target.value)}
                        className="bg-brand-dark border border-brand-green/50 text-brand-green font-mono rounded p-2 w-24 outline-none h-10"
                        placeholder="1.65"
                        step="0.01"
                        disabled={dailyResult && dailyResult.status !== 'pending'}
                      />
              ...
              <button onClick={handleSaveStake} className="p-2 h-10 bg-brand-gray rounded hover:bg-brand-green hover:text-black transition-colors" title="Salvar">
                  <Save size={20} />
              </button>
          </div>
          ...
          <div className="flex items-center gap-4 bg-brand-dark p-4 rounded-lg border border-brand-gray min-w-[200px] justify-center h-24">
              {dailyResult && dailyResult.status !== 'pending' ? (
                  <div className="text-center animate-in fade-in zoom-in">
                      ...
                      <div className={`text-3xl font-bold ${dailyResult.status === 'green' ? 'text-brand-green' : 'text-red-500'}`}>
                          {dailyResult.status === 'green' ? '+' : ''}R$ {dailyResult.profit?.toFixed(2)}
                      </div>
      ...
      {activeMethod === 'under_85' && (
          <div className="mb-4 flex items-center gap-2 text-yellow-400 text-sm bg-yellow-400/10 p-3 rounded-lg border border-yellow-400/20">
              <AlertCircle size={16} />
              <span>Para "Under 85 min", defina manualmente o status (WIN/LOSS) de cada jogo selecionado.</span>
          </div>
      )}
      ...
      <div className="flex-1 overflow-auto bg-brand-card rounded-xl border border-brand-gray">
        <table className="w-full text-left border-collapse">
            <thead className="bg-brand-dark sticky top-0 text-xs uppercase text-text-secondary font-bold z-10">
                <tr>
                    <th className="p-3 w-10 text-center"></th> {/* Coluna para o Chevron */}
                    ...
                    entries.map((entry, idx) => (
                        <React.Fragment key={entry.id}>
                            ...
                            <tr className={`border-b border-brand-gray/50 ${entry.is_main_pick ? 'bg-brand-green/5' : 'hover:bg-brand-gray/20'}`}>
                                ...
                                <td className="p-3 text-center">
                                    <button 
                                        onClick={() => toggleMainPick(entry)}
                                        className={`p-2 rounded-full transition-colors ${
                                            entry.is_main_pick ? 'bg-brand-green text-brand-dark shadow-glow' : 'bg-brand-gray text-text-secondary hover:text-white'
                                        }`}
                                        title="Selecionar como Jogo Principal"
                                    >
                                        <Target size={20} />
                                    </button>
                                </td>
                                <td className="p-3">
                                    <div className="font-mono text-xs text-text-secondary">
                                        {entry.games.match_time ? entry.games.match_time.substring(0,5) : '--:--'}
                                    </div>
                                    <div className="font-bold text-white text-lg">
                                        {entry.games.home_team || 'Time Casa'} 
                                        <span className="text-text-secondary text-sm mx-1">vs</span> 
                                        {entry.games.away_team || 'Visitante'}
                                    </div>
                                    <div className="text-xs text-brand-green">
                                        {entry.games.country ? `${entry.games.country} - ` : ''}
                                        {entry.games.league || 'Liga não Inf.'}
                                    </div>
                                </td>

                                {activeMethod === 'my_games' && (
                                    <td className="p-3 text-center">
                                        <span className="px-2 py-1 rounded bg-brand-gray border border-brand-gray text-white text-xs font-bold uppercase">
                                            {entry.selection?.replace(/_/g, ' ').replace('win', 'Vence').replace('draw', 'Empate') || 'Manual'}
                                        </span>
                                    </td>
                                )}
                                
                                <td className="p-3 text-center font-mono text-text-secondary">
                                    {entry.games.home_score_ht !== null 
                                        ? `${entry.games.home_score_ht}-${entry.games.away_score_ht}` 
                                        : '-'}
                                </td>

                                <td className="p-3 text-center font-bold font-mono text-xl">
                                    {entry.games.status === 'FT' || entry.games.home_score !== null
                                ...
                                <td className="p-3 text-center">
                                    <div className="flex justify-center gap-1">
                                        <button 
                                            onClick={() => setGameStatus(entry.id, 'win')}
                                            className={`p-1.5 rounded flex items-center gap-1 transition-all ${
                                                entry.result_status === 'win' 
                                                ? 'bg-green-500 text-black font-bold' 
                                                : 'bg-brand-gray text-text-secondary hover:bg-green-500/20 hover:text-green-400'
                                            }`}
                                        >
                                            <Check size={14} /> 
                                        </button>
                                        ...
                                        <button 
                                            onClick={() => setGameStatus(entry.id, 'loss')}
                                            className={`p-1.5 rounded flex items-center gap-1 transition-all ${
                                                entry.result_status === 'loss' 
                                                ? 'bg-red-500 text-white font-bold' 
                                                : 'bg-brand-gray text-text-secondary hover:bg-red-500/20 hover:text-red-400'
                                            }`}
                                        >
                                            <X size={14} />
                                        </button>
                            ...
                            {/* LINHA EXPANDIDA COM ESTATÍSTICAS */}
                                    ...
                                    <td colSpan="8" className="p-4">
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-sm animate-in slide-in-from-top-2">
                                            ...
                                            <div className="space-y-2 border-r border-brand-gray pr-4">
                                                <h4 className="text-brand-green font-bold text-xs uppercase flex items-center gap-1">
                                                    <Activity size={14} /> Odds & Probabilidades
                                                </h4>
                                                <div className="grid grid-cols-2 gap-2 text-xs">
                                                    <div>Home: <span className="text-white font-mono">{entry.games.odd_home?.toFixed(2) || '-'}</span></div>
                                                    <div>Away: <span className="text-white font-mono">{entry.games.odd_away?.toFixed(2) || '-'}</span></div>
                                                    <div>O 2.5: <span className="text-white font-mono">{entry.games.odd_over25?.toFixed(2) || '-'}</span></div>
                                                    <div>U 3.5: <span className="text-white font-mono">{entry.games.odd_under35?.toFixed(2) || '-'}</span></div>
                                            ...
                                            {/* Bloco 2: Desempenho */}
                                            <div className="space-y-2 border-r border-brand-gray pr-4">
                                                <h4 className="text-brand-green font-bold text-xs uppercase">Desempenho Geral</h4>
                                                <div className="space-y-1 text-xs">
                                                    <div className="flex justify-between">
                                                        <span className="text-text-secondary">Eficiência:</span>
                                                        <span className="text-white">{entry.games.efficiency_home}% <span className="text-text-secondary">vs</span> {entry.games.efficiency_away}%</span>
                                                    </div>
                                                    <div className="flex justify-between">
                                                        <span className="text-text-secondary">Ranking:</span>
                                                        <span className="text-white">#{entry.games.rank_home} <span className="text-text-secondary">vs</span> #{entry.games.rank_away}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            {/* Bloco 3: Gols (Médias) */}
                                            <div className="space-y-2 border-r border-brand-gray pr-4">
                                                <h4 className="text-brand-green font-bold text-xs uppercase">Médias de Gols</h4>
                                                <div className="space-y-1 text-xs">
                                                    <div className="flex justify-between">
                                                        <span className="text-text-secondary">Marcados:</span>
                                                        <span className="text-white font-mono">{entry.games.avg_goals_scored_home} <span className="text-text-secondary">|</span> {entry.games.avg_goals_scored_away}</span>
                                                    </div>
                                                    <div className="flex justify-between">
                                                        <span className="text-text-secondary">Sofridos:</span>
                                                        <span className="text-white font-mono">{entry.games.avg_goals_conceded_home} <span className="text-text-secondary">|</span> {entry.games.avg_goals_conceded_away}</span>
                                                    </div>
                                                    <div className="flex justify-between text-yellow-400">
                                                        <span>Sofr. 2ºT:</span>
                                                        <span className="font-mono">{entry.games.avg_goals_conceded_2h_home} <span className="text-text-secondary">|</span> {entry.games.avg_goals_conceded_2h_away}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="space-y-2">
                                                <h4 className="text-brand-green font-bold text-xs uppercase">Estatística Global</h4>
                                                <div className="space-y-1 text-xs">
                                                    <div className="flex justify-between">
                                                        <span className="text-text-secondary">Gols/Jogo (Match):</span>
                                                        <span className="text-white font-mono font-bold">{entry.games.global_goals_match}</span>
                                                    </div>
                                                    <div className="flex justify-between">
                                                        <span className="text-text-secondary">Média Liga:</span>
                                                        <span className="text-white font-mono">{entry.games.global_goals_league}</span>
                                                    </div>
                                                </div>
                                            </div>
